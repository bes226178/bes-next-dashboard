...
export default function TenureHeatmap({ employees = [] }) {
  // 處理資料並計算每個入職年度和職等組合的平均在職年資
  const heatmapData = useMemo(() => {
    ...
    const sortedYears = Array.from(startYears).sort((a, b) => a - b)
    // 依 GRADE_ID 由大到小排序職等
    ...
    employees.forEach(emp => {
      ...
      if (year != null && !isNaN(year) && grade && typeof grade === 'string' && tenure > 0) {
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        if (detail) {
          return `入職年度: ${year}年<br/>職等: ${grade}<br/>人數: ${detail.count} 人<br/>平均在職年資: ${detail.avgTenure} 年<br/>最小年資: ${detail.minTenure} 年<br/>最大年資: ${detail.maxTenure} 年`
        }
        return `入職年度: ${year}年<br/>職等: ${grade}<br/>平均在職年資: ${params.data[2]} 年`
      },
    },
    grid: {
      height: '60%',
    ...
    <Box sx={{ mt: 0 }}>
      ...
      >
        ...
        >
          ...
          >
            此熱力圖以入職年度與職等交叉分析平均在職年資，揭示不同職等層級的人才留存模式。深色區域（高年資）顯示該入職年度與職等組合具有較強的留才能力，可能與招募品質、職涯發展路徑或組織文化契合度相關；淺色區域（低年資）則標示高風險組合，需優先檢視該群體的離職原因。橫向比較可觀察特定職等在不同入職年度的留存差異，縱向比較則反映特定入職年度在不同職等的發展軌跡。若特定職等或入職年度持續呈現低年資，建議深入分析該群體的職涯發展、薪酬競爭力與工作滿意度，並制定針對性的留才方案。
          </Typography>
        </Box>
      </Box>
    </Box>
